...
if (localStorage.getItem("quarto")) {
    switch (localStorage.getItem("quarto")) {
        ...
        case "premium":
            ...
            document.querySelector("#bookingValor_modal").innerText = `R$ ${(quartos[2]["valor"] * qtdAtual).toFixed(2)}`
            document.querySelector("#premium").checked = true
            break
        default:
            document.querySelector("#bookingApt").innerText = "Executive"
            document.querySelector("#bookingValor_modal").innerText = `R$ ${(quartos[0]["valor"] * qtdAtual).toFixed(2)}`
...
$("#continuar").click(function() {
    ...
    if (
        ...
        document.querySelector("#bookingApt_modal").innerText = localStorage.getItem("quarto")[0].toUpperCase() + localStorage.getItem("quarto").substr(1)
        document.querySelector("#bookingCheckIn_modal").innerText = localStorage.getItem("checkin")
        document.querySelector("#bookingCheckOut_modal").innerText = localStorage.getItem("checkout")
        document.querySelector("#bookingQtd_modal").innerText = localStorage.getItem("qtdPessoas")

        switch (localStorage.getItem("quarto")) {
            case "classic":
                document.querySelector("#modal-quarto p:nth-of-type(1)").innerText = quartos[1]["descricao"]
                document.querySelector("#modal-quarto p:nth-of-type(2)").innerText = `R$ ${(quartos[1]["valor"]).toFixed(2)}`
                document.querySelector("#bookingImg_modal").src = '/images/acomodacoes/acomodacoes-luxo-1.png'
                document.querySelector("#bookingValor_modal").innerText = `R$ ${(quartos[1]["valor"] * qtdAtual).toFixed(2)}`
                break
            case "premium":
                document.querySelector("#modal-quarto p:nth-of-type(1)").innerText = quartos[2]["descricao"]
                document.querySelector("#modal-quarto p:nth-of-type(2)").innerText = `R$ ${(quartos[2]["valor"]).toFixed(2)}`
                document.querySelector("#bookingImg_modal").src = '/images/acomodacoes/acomodacoes-standard-1.png'
                document.querySelector("#bookingValor_modal").innerText = `R$ ${(quartos[2]["valor"] * qtdAtual).toFixed(2)}`
                break
            default:
                document.querySelector("#modal-quarto p:nth-of-type(1)").innerText = quartos[0]["descricao"]
                document.querySelector("#modal-quarto p:nth-of-type(2)").innerText = `R$ ${(quartos[0]["valor"]).toFixed(2)}`
                document.querySelector("#bookingImg_modal").src = '/images/acomodacoes/acomodacoes-presidencial-1.png'
                document.querySelector("#bookingValor_modal").innerText = `R$ ${(quartos[0]["valor"] * qtdAtual).toFixed(2)}`
                break
        }
    }
